...
-- Définissez les variable d'environnement LINUX suivante :
-- MYSPID, MYINST, TRACEFILESPATH, MYTKPROFUSER
----------------------------------------------------------------------------------

##################################################################################
# Ouvrir un CMD et lancer vagrant ssh afin de pour exécuter tkprof 
##################################################################################
...
-- Devenir Oracle afin de pouvoir lancer tkprof
sudo -su oracle


-- Définir la variable d'environnement LINUX qui va contenir la valeur de vp.spid
-- Qui va permettre d'identifier le fichier qui contient la trace tkprof 
export MYSPID=8621

-- Définir la variable d'environnement LINUX qui va contenir le nom de l'instance
export MYDBNAME=orclcdb
export MYINST=ORCLCDB

-- Définir la variable d'environnement LINUX qui contient le nom user à utiliser 
-- Lors du lancement de tkprof
export MYTKPROFUSER=ORS1

-- Définir la variable d'environnement LINUX qui contient le chemin vers les trace 
-- Oracle. 
export TRACEFILESPATH=$ORACLE_BASE/diag/rdbms/$MYDBNAME/$MYINST/trace

export MYTKPROFALIAS=ORCLPDB1

--- CAS 1 : Vous avez une base Oracle Locale et vous travaillez avec Oracle 11g
-- ou 12c. Lancer tkprof sous LINUX comme suit :
#tkprof $TRACEFILESPATH/$MYINST_ora_$MYSPID.trc Exo41_trc.trc EXPLAIN=%MYTKPROFUSER%@%MYTKPROFALIAS%/PassOrs1 SORT = execpu, fchcpu sys=n

tkprof $TRACEFILESPATH/$MYINST\_ora\_$MYSPID.trc Exo41_trc.trc SORT = execpu, fchcpu sys=n
...
##################################################################################
# Ouvrir un CMD et lancer vagrant ssh afin pour exécuter expdp 
##################################################################################
...
-- Devenir Oracle afin de pouvoir lancer expdp
sudo -su oracle
...
?
[oracle@oracle-21c-vagrant vagrant]$ expdp $DPUSER/PassOrs1@$DPDBALIAS dumpfile=stators.dump tables=STATors

Export: Release 21.0.0.0.0 - Production on Thu Mar 14 20:59:17 2024
Version 21.3.0.0.0

Copyright (c) 1982, 2021, Oracle and/or its affiliates.  All rights reserved.

Connected to: Oracle Database 21c Enterprise Edition Release 21.0.0.0.0 - Production
Starting "ORS1"."SYS_EXPORT_TABLE_01":  ORS1/********@ORCLPDB1 dumpfile=stators.dump tables=STATors
...
. . exported "ORS1"."STATORS"                            25.75 KB      88 rows
Master table "ORS1"."SYS_EXPORT_TABLE_01" successfully loaded/unloaded
******************************************************************************
Dump file set for ORS1.SYS_EXPORT_TABLE_01 is:
  /opt/oracle/admin/ORCLCDB/dpdump/EED6D9ACDC237A0BE0530101007F32A4/stators.dump
Job "ORS1"."SYS_EXPORT_TABLE_01" successfully completed at Thu Mar 14 20:59:49 2024 elapsed 0 00:00:28

[oracle@oracle-21c-vagrant vagrant]$
